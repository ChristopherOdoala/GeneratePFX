using GeneratePFX.CertificateManager;
...
namespace GeneratePFX
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var rsaCert = CreateRsaCertificate("localhost", 90);
            var ecdsaCert = CreateECDsaCertificate("localhost", 90);

            string password = "1234";
            var iec = sp.GetService<ImportExportCertificate>();

            var rsaCertPfxBytes = iec.ExportSelfSignedCertificatePfx(password, rsaCert);
            File.WriteAllBytes("/home//sbscadmin//datadrive//Filestore//auth//auth.pfx", rsaCertPfxBytes);

            var ecdsaCertPfxBytes = iec.ExportSelfSignedCertificatePfx(password, ecdsaCert);
            File.WriteAllBytes("/home//sbscadmin//datadrive//Filestore//auth//cert_ecdsa384.pfx", ecdsaCertPfxBytes);
